...
describe('Invoice Entity', () => {
  ...
  const validItems = [
    new InvoiceItem('Widget', 5, new Money(10.0)),
    new InvoiceItem('Gadget', 2, new Money(25.0)),
  ];
  ...
  describe('Invoice Creation', () => {
    ...
    it('should calculate subtotal correctly', () => {
      const invoice = new Invoice(validInvoiceData);
      // 5 * 10 + 2 * 25 = 50 + 50 = 100
      expect(invoice.getSubtotal().getAmount()).toBe(100.0);
    });

    it('should apply minimum invoice amount of $25', () => {
      const smallItems = [new InvoiceItem('Small Item', 1, new Money(10.0))];
      const invoice = new Invoice({
        ...validInvoiceData,
        items: smallItems,
      });

      expect(invoice.getSubtotal().getAmount()).toBe(25.0);
    });
  });
  ...
  describe('Validation', () => {
    ...
    it('should throw error for negative amounts', () => {
      ...
      expect(() => {
        new InvoiceItem('Bad Item', 1, new Money(-10.0));
  ...
  describe('Tax Calculation', () => {
    ...
    it('should use default tax rate for unknown states', () => {
      ...
      const tax = invoice.calculateTax();
      expect(tax.getAmount()).toBe(5.0); // 100 * 0.05
    });
  });

  describe('Bulk Discount', () => {
    it('should apply 3% bulk discount for orders >= $10,000', () => {
      const bulkItems = [new InvoiceItem('Bulk Item', 1000, new Money(10.0))];
      const invoice = new Invoice({
        ...validInvoiceData,
        items: bulkItems,
      });
      ...
      expect(invoice.getSubtotal().getAmount()).toBe(10000.0);
      expect(invoice.getBulkDiscount().getAmount()).toBe(300.0);
    ...
    it('should not apply bulk discount for orders < $10,000', () => {
      const smallBulkItems = [new InvoiceItem('Item', 999, new Money(10.0))];
      const invoice = new Invoice({
        ...validInvoiceData,
        items: smallBulkItems,
      });

      expect(invoice.getSubtotal().getAmount()).toBe(9990.0);
  ...
  describe('Immutability', () => {
    it('should not allow modification of customer after creation', () => {
      ...
      expect((invoice as unknown as Record<string, unknown>)['setCustomer']).toBeUndefined();
